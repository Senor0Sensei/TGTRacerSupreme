...
//filename is file output by MATLAB to draw. Should end in *.pic
void FEHImage::Open(const char *filename)
{	
	char file[19] = ".\\Textures\\";
	char name[60];
...
void FEHImage::Draw(int x, int y, int a)
{
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
		{
			if (*(saved_image + (i * cols) + j) != -1)
			{
				...
				LCD.DrawPixel(j + x, i + y, a);
			}
		}
	}
}

void FEHImage::DrawLine(int x, int y, int i, int a)
{
	for (int j = 0; j < cols; j++)
	{
		if (*(saved_image + (i * cols) + j) != -1)
		{
			LCD.SetFontColor(*(saved_image + (i * cols) + j));
			LCD.DrawPixel(j + x, y, a);
		}
	}
}

void FEHImage::DrawAtlas(int x, int y, int px, int py, int width, int height, int a) 
{
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			if (*(saved_image + ((i + py) * cols) + (j + px)) != -1)
			{
				LCD.SetFontColor(*(saved_image + ((i + py) * cols) + (j + px)));
				LCD.DrawPixel(j + x, i + y, a);
			}
		}
	}
