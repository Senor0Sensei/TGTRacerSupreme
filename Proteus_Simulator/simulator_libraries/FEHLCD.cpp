...
void FEHLCD::DrawPixel(int x, int y, int a)
{
...
void FEHLCD::DrawHorizontalLine(int y, int x1, int x2, int a)
{
    if (x2 < x1) {
        Swap(&x1, &x2);
    }
...
void FEHLCD::DrawVerticalLine(int x, int y1, int y2, int a)
{
    if (y2 < y1) {
        Swap(&y1, &y2);
    }
...
void FEHLCD::DrawLine(int x1, int y1, int x2, int y2, int a)
{
    if (x1 == x2) {
        DrawVerticalLine(x1, y1, y2, a);
        return;
    }
    if (y1 == y2) {
        DrawHorizontalLine(y1, x1, x2, a);
        return;
    }
    ...
    for (xVal = startX; xVal <= endX; xVal++)
    {
        ...
        {
            DrawPixel(xVal, yVal, a);
        }
    }
}

void FEHLCD::DrawRectangle(int x, int y, int width, int height, int a)
{
    DrawHorizontalLine(y, x, x + width - 1, a);
    DrawHorizontalLine(y + height - 1, x, x + width - 1, a);
    DrawVerticalLine(x, y, y + height - 1, a);
    DrawVerticalLine(x + width - 1, y, y + height - 1, a);
}

void FEHLCD::FillRectangle(int x, int y, int width, int height, int a)
{
    for (int currentX = x; currentX < x + width; currentX++)
    {
        for (int currentY = y; currentY < y + height; currentY++)
        {
            DrawPixel(currentX, currentY, a);
        }
    }
}

void FEHLCD::DrawCircle(int x0, int y0, int r, int a)
{
    ...
    while (x < y)
    {
        ...
        ddF_x += 2;
        f += ddF_x;
        DrawPixel(x0 + x, y0 + y, a);
        DrawPixel(x0 - x, y0 + y, a);
        DrawPixel(x0 + x, y0 - y, a);
        DrawPixel(x0 - x, y0 - y, a);
        DrawPixel(x0 + y, y0 + x, a);
        DrawPixel(x0 - y, y0 + x, a);
        DrawPixel(x0 + y, y0 - x, a);
        DrawPixel(x0 - y, y0 - x, a);
    }
...
void FEHLCD::FillCircle(int x0, int y0, int r, int a)
{
    ...
    DrawVerticalLine(x0, y0 - r, y0 + r, a);
    DrawHorizontalLine(y0, x0 - r, x0 + r, a);
    ...
    while (x < y)
    {
        ...
        ddF_x += 2;
        f += ddF_x;
        DrawHorizontalLine(y0 + x, x0 - y, x0 + y, a);
        DrawHorizontalLine(y0 - x, x0 - y, x0 + y, a);
        DrawVerticalLine(x0 + x, y0 - y, y0 + y, a);
        DrawVerticalLine(x0 - x, y0 - y, y0 + y, a);
    }
}

// Write information at a specific Pixel on the screen
void FEHLCD::WriteAt(const char *str, int x, int y, int a)
{
    int i = 0;
    while (str[i] != '\0')
    {
        WriteCharAt(x, y, str[i], a);
        x += CHAR_WIDTH;
        i++;
    }
}

void FEHLCD::WriteAt(std::string str, int x, int y, int a)
{
    WriteAt(str.c_str(), x, y, a);
}

void FEHLCD::WriteAt(int i, int x, int y, int a)
{
    char num[50];
    sprintf(num, "%d", i, a);
    WriteAt(num, x, y, a);
...
void FEHLCD::WriteAt(float f, int x, int y, int a)
{
    char num[50];
    int d, r;
    d = (int)f;
...
void FEHLCD::WriteAt(double d, int x, int y, int a)
{
    WriteAt((float)d, x, y, a);
}

void FEHLCD::WriteAt(bool b, int x, int y, int a)
{
    if (b)
    {
        WriteAt("true", x, y, a);
...
void FEHLCD::WriteAt(char c, int x, int y, int a)
{
    WriteCharAt(x, y, c, a);
}
...
void FEHLCD::WriteRC(const char *str, int row, int col, int a)
{
    int x, y;
...
void FEHLCD::WriteRC(int i, int row, int col, int a)
{
    int x, y;
...
void FEHLCD::WriteRC(float f, int row, int col, int a)
{
    int x, y;
...
void FEHLCD::WriteRC(double d, int row, int col, int a)
{
    int x, y;
...
void FEHLCD::WriteRC(bool b, int row, int col, int a)
{
    int x, y;
...
void FEHLCD::WriteRC(char c, int row, int col, int a)
{
    int x, y;
...
// Write to the screen
void FEHLCD::Write(const char *str, int a)
{
    int i = 0;
    CheckLine();
    while (str[i] != '\0')
    {
        WriteChar(_currentline, _currentchar, str[i], a);
        NextChar();
        i++;
    }
}

void FEHLCD::Write(int i, int a)
{
    char num[50];
    sprintf(num, "%d", i);
    Write(num, a);
...
void FEHLCD::Write(float f, int a)
{
    char num[50];
    int d, r;
    if (f >= 0)
...
void FEHLCD::Write(double d, int a)
{
    Write((float)d, a);
}

void FEHLCD::Write(bool b, int a)
{
    if (b)
    {
        Write("true", a);
...
void FEHLCD::Write(char c, int a)
{
    CheckLine();
    WriteChar(_currentline, _currentchar, c, a);
    NextChar();
}

// Write to the screeen and advance to next line
void FEHLCD::WriteLine(const char *str, int a)
{
    CheckLine();
    Write(str, a);
    NextLine();
...
void FEHLCD::WriteLine(std::string str, int a)
{
    WriteLine(str.c_str(), a);
}

void FEHLCD::WriteLine(int i, int a)
{
    CheckLine();
    Write(i, a);
    NextLine();
...
void FEHLCD::WriteLine(float f, int a)
{
    CheckLine();
    Write(f, a);
    NextLine();
...
void FEHLCD::WriteLine(double d, int a)
{
    CheckLine();
    Write(d, a);
    NextLine();
...
void FEHLCD::WriteLine(bool b, int a)
{
    CheckLine();
    Write(b, a);
    NextLine();
...
void FEHLCD::WriteLine(char c, int a)
{
    CheckLine();
    Write(c, a);
    NextLine();
...
void FEHLCD::WriteChar(int row, int col, char c, int a)
{
    WriteCharAt(2 + col * CHAR_WIDTH, 3 + row * CHAR_HEIGHT, c, a);
}

void FEHLCD::WriteCharAt(int x, int y, char c, int a)
{
    // Force X and Y to be positive
    while (x < 0)
    {
    ...
    for (int col = 0; col < 5; col++)
    {
        for (int row = 0; row < 7; row++)
        {
            ...
            if (((charData[col] >> row) & 0x01) == 1)
            {
                ...
                FillRectangle(x + col * 2, y + row * 2, 2, 2, a);
            }
        }
    }
}
...
namespace FEHIcon
{
    ...
    void Icon::Draw()
    {
        ...
        LCD.DrawRectangle(x_start, y_start, width, height, 0);
        LCD.SetFontColor(textcolor);
        LCD.WriteAt(label, x_start + ((width - (strlen(label) * 12)) / 2), y_start + ((height - 17) / 2), 0); // equation to center text inside the icon
    }
    ...
    {
        LCD.SetFontColor(color);
        LCD.DrawRectangle(x_start + 1, y_start + 1, width - 2, height - 2, 0);
        LCD.DrawRectangle(x_start + 2, y_start + 2, width - 4, height - 4, 0);
        LCD.DrawRectangle(x_start + 3, y_start + 3, width - 6, height - 6, 0);
        set = 1;
    }
    ...
    void Icon::ChangeLabelString(const char new_label[])
    {
        if (strcmp(label, new_label))
        {
            ...
            LCD.SetFontColor(BLACK);
            LCD.FillRectangle(x_start + 1, y_start + 1, width - 2, height - 2, 0);
            Draw();
        }
    }
